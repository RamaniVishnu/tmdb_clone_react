...
function Favourite(){
    ...
    return (
        ...
        {data.map((data,index)=><div><div key={index} className="favourite_card"><img className="favourite_img" src={IMAGE_PATH+data}></img></div><button style={{background: "white",
    border: "none",
    display: "flex",
    margin: "5px auto",
    ...
    color: "#959595",
    fontSize: "20px"}}>Remove</p></button></div>)}
        </div>
        <Footer />
        </>
    )
